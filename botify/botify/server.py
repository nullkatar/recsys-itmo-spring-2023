...
class NextTrack(Resource):
    def post(self, user: int):
        ...
        if treatment == Treatment.T1:
            recommender = HOMEWORK(tracks_redis.connection, recommendations_redis.connection, catalog,
                                   app.config["USER_EMBS"], app.config["CONTEXT_EMBS"], app.config["TRACK_EMBS"])
        else:
